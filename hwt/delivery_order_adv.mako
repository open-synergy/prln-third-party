...
<%column_width_in_chars=[3,80,7,6,6,1]%>            <!--Lebar masing-masing kolom line item dalam jumlah huruf/karakter-->
...
<%preprinted_header_space = 10%>                   <!--Blank space untuk area preprinted dari formulir-->
...
%for document in objects :
    ...
    %if state not in ('confirmed', 'assigned', 'done'):
...
<!--================================================================== EDITABLE AREA ===============================================================-->
	    %if prod.state not in ['cancel']:
		    ...
		    %if prod.product_id.dus:
		        <%str_holder.append('%.2f' %(prod.product_qty/float(prod.product_id.dus)))%>
		        <%totdus += int(math.ceil(prod.product_qty/prod.product_id.dus))%>
		    %else:
		        <%str_holder.append('')%>
		    %endif
		    <!--Kolom 6: nama satuan box produk-->
		    <%xweight=0%>
		    <%xweight=prod.product_qty*prod.product_id.weight_net%>
		    <!-%str_holder.append(formatLang(prod.product_qty*prod.product_id.weight, digits=0) or '')%-->
		    <%totweight += float(prod.product_id.weight_net*prod.product_qty)%>
...
####################################################################################################################################################-->
        ...
        <tr>
            <td>
				...
				%if document.sale_id and document.sale_id.client_order_ref:
                                        <%nodoc = str(document.sale_id and document.sale_id.client_order_ref[:20]) or ''%>
                                        No. Order : ${nodoc}
                                %else:
                                        No. Order :
                                %endif
                                ...
                                <table class="table" cellpadding="2">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="width: 70%;" class="table_border">Nama Barang / Ukuran</td>
                                            <td style="width: 10%;" class="table_border">Jumlah</td>
                                            <td style="width: 8%;"  class="table_border">Satuan</td>
                                            <td style="width: 8%;" class="table_border" colspan="2">Box</td>
                                        </tr>
                ...
                %for line_no in range (current_line,last_line):
                    %if line_no < total_lines_print:
                            ...
                            <tr>
                                ...
                                <td class="table_line" style="text-align: right; height:${line_item_height}px;" colspan="2">
                                ${line_item_print[line_no][4]} ${line_item_print[line_no][5]}
                                </td>
                                <!--td class="table_line" style="text-align: right; height:${line_item_height}px; width: 250px;">
...
####################################################################################################################################################-->
        ...
        <tr>
            <td colspan="6" rowspan="1" style="padding-left:4px;">
                <div style="height: 65px;">
                    <p class="diva"><b>KETERANGAN : Total : ${totdus} box dan ${formatLang(totweight, digits=2)} kg</b><br />${document.note and document.note[0:400] or ''|entity}</p>
                </div>
            </td>
        </tr>
        ...
        <tr>
			...
			<br>Tgl Konfirmasi SO : ${document.sale_id.date_confirm or ''|entity}
            </td>
                </table>
                </div>
            </td>
        ...
        <tr style="display: none;">
            ...
            <td style="text-align: right; vertical-align: bottom; width: 750px;">
                pagex ${current_page} of ${total_page}
            </td>
            %else:
            <td style="text-align: right; vertical-align: bottom; width: 750px;">
                pagex ${current_page} of ${total_page}
